...
loop:
    ...
    BTFSC ((PORTB) and 07Fh), 0 ; Revismos si el bit 0 de PORTB está en 0
    call inc_portD ; NO: está presionado, llamar a incrementar D
    BTFSC ((PORTB) and 07Fh), 1 ; SÍ: no está presionado, revisar si el bit 1 de PORTB está en 0
...
timer0_counter:
    BTFSS ((INTCON) and 07Fh), 2 ; Chequeamos si la bandera del timer se ha activado
...
inc_portD:
    BTFSC ((PORTB) and 07Fh), 0 ; Revisamos si el pin 0 del puerto B está en 0
...
dec_portD:
    BTFSC ((PORTB) and 07Fh), 1 ;Revisamos si el pin 1 del puerto B está en 0
    goto $-1 ;NO: el botón sigue presionado, regresar al chequeo
